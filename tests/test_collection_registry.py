...
class _FakeStore:
    ...
    def upsert_rag_collection(self, *, collection_name: str, ontology_id: str):
        self.rows[collection_name] = ontology_id
        return {"collection_name": collection_name, "ontology_id": ontology_id}

    def get_rag_collection(self, *, collection_name: str):
        oid = self.rows.get(collection_name)
        if oid is None:
            return None
        return {"collection_name": collection_name, "ontology_id": oid}
...
def test_persistent_registry_create_and_get() -> None:
    ...
    assert b.collection_name == "stocks:nse:INFY"
...
def test_persistent_registry_ensure_binding_and_list() -> None:
    ...
    assert [r.collection_name for r in rows] == ["stocks:nse:INFY", "stocks:nse:TCS"]
